...
import { TroveApproveABI } from "@/design-systems/web3Utils/TroveApprovalABI";
import { BorrowerABI } from "@/design-systems/web3Utils/BorrowerABI";
import {
  BorrowerContractAdd,
...
const SelectVaultForm: React.FC<VaultsFormProps> = ({urlAddress}) => {
  ...
  const handleOpenTrove = async () => {
    try {
      ...
      const _maxFeePercentage = ethers.utils.parseUnits(amount.toString(), 16);
      const _collateralAmount = ethers.utils.parseUnits(amount.toString(), 16);
        ...
        <div className="w-full px-6 pb-4">
          ...
          <div className="bg-[#F7F7F7] text-md flex flex-col gap-1 font-medium text-darkBlue mt-4 rounded-sm p-2">
            ...
            <div className="flex justify-between">
              ...
              <Typography>
                (0.64%) 0.00 {data?.prismaType === "prisma" ? "mkUSD" : "ULTRA"}
              </Typography>
            </div>
            <div className="flex justify-between">
          ...
          {chainId != 11155111 ? (
            <Typography className="text-darkBlue text-body font-normal text-center mt-3">
              Please connect to Ethereum Mainnet in your wallet.
            </Typography>
          ) : (
            <></>
          )}
